...
inline int inject_energy ( const int     periodic,      // flag for periodic boundary conditions
                           const int     Nsources,      // number of sources
			   const int    *Sources,                   // array of source coordinates
			   const double  energy,                    // energy to inject
			   const int     mysize[2],                 // size of the grid
                           double *plane )              // pointer to the grid
{
   #define IDX( i, j ) ( (j)*(mysize[_x_]+2) + (i) )        // flatten the point froma 2D -> 1D coordinates, transform the coordnates
    for (int s = 0; s < Nsources; s++) {
        
        int x = Sources[2*s];
        int y = Sources[2*s+1];
        plane[IDX(x, y)] += energy;             // inject energy in the grid with the host border (halo)
   ...
   #define IDX( i, j ) ( (j)*fxsize + (i) )
    ...
    if ( periodic )
         ...
         */
        {
            for ( int i = 1; i <= xsize; i++ )
                {
                    new[ i ] = new[ IDX(i, ysize) ];            // copy the last row into the first ghost row
                    new[ IDX(i, ysize+1) ] = new[ i ];          // copy the first row into the last ghost row
                }
            for ( int j = 1; j <= ysize; j++ )
                {
                    new[ IDX( 0, j) ] = new[ IDX(xsize, j) ];       // copy the last column into the first ghost column
                    new[ IDX( xsize+1, j) ] = new[ IDX(1, j) ];     // copy the first column into the last ghost column
                }
        }
