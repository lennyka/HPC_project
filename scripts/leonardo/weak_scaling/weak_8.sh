...
start_time=$(date +%s.%N)
srun --ntasks=$nt --cpus-per-task=14 --cpu-bind=cores  ./main -x $x -y $y -p 0 -o 0 -e 300 -v 1 > ./output/leonardo/weak_scaling/output_weak_8_node_8taskpernode_14cpupertask.log
end_time=$(date +%s.%N)
