...
int main(int argc, char **argv)
{
  ...
  int  periodic;              // periodic boundary conditions flag: 0 if not periodic, 1 if periodic
  int  S[2];                  // grid size (x, y)

  int     Nsources;           // number of heat sources
  int    *Sources;            // array of heat source coordinates
  double  energy_per_source;  // energy to inject per source

  double *planes[2];          // array of pointers to the energy planes

  double injected_heat = 0;   // total injected heat

  int injection_frequency;    // frequency of heat injection
  int output_energy_at_steps = 0; // whether to output the energy budget at every step
  ...
  for (int iter = 0; iter < Niterations; iter++)
    ...
    {      
      ...
      if ( output_energy_at_steps )     // if we want to output energy at every step
        {
          double system_heat;
 ...
 */
{
  ...
  planes_ptr[OLD] = (double*)malloc( 2*bytes*sizeof(double) );     // allocate memory for both planes
  memset ( planes_ptr[OLD], 0, 2*bytes*sizeof(double) );           // initialize memory to zero
  planes_ptr[NEW] = planes_ptr[OLD] + bytes;                       // set pointer for new plane
